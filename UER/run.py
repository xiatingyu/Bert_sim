...
def read_dataset(args, path, isshuffle=False):
    ...
    if isshuffle == True:
        ...
        text_a, text_b, label = data['text_a'], data['text_b'], data['labels']
        proportion = args.proportion
        text_a, text_b, label = text_a.iloc[:int(len(text_a) * proportion)], text_b.iloc[:int(len(text_b) * proportion)], \
                                label.iloc[:int(len(label) * proportion)]
    else:
        text_a, text_b, label = data['text_a'], data['text_b'], data['labels']
...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    if print_confusion_matrix:
        ...
        final_pred['index'] = range(len(prediction))
        final_pred['prediction'] = prediction
...
def freeze(model):
    unfreeze_layers = ['output_layer', 'transformer.6.']
