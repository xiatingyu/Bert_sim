...
class Classifier(nn.Module):
    ...
    def forward(self, src, tgt, seg, sim):
        """
        Args:
...
def batch_loader(batch_size, src, tgt, seg, sim):
    ...
    for i in range(instances_num // batch_size):
        ...
        yield src_batch, tgt_batch, seg_batch, sim_batch
...
def train_model(args, model, optimizer, scheduler, src_batch, tgt_batch, seg_batch, sim_batch):
    model.zero_grad()
    ...
    loss, _ = model(src_batch, tgt_batch, seg_batch, sim_batch)
...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    args.model.eval()
    preds, golds = [], []
    for i, (src_batch, tgt_batch, seg_batch, sim_batch) in enumerate(batch_loader(batch_size, src, tgt, seg, sim)):
        src_batch = src_batch.to(args.device)
        tgt_batch = tgt_batch.to(args.device)
        seg_batch = seg_batch.to(args.device)
        sim_batch = sim_batch.to(args.device)
...
def main():
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        for i, (src_batch, tgt_batch, seg_batch, sim_batch) in enumerate(batch_loader(batch_size, src, tgt, seg, similarity)):
            loss = train_model(args, model, optimizer, scheduler, src_batch, tgt_batch, seg_batch, sim_batch)
