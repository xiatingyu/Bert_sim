...
from esim.data_sim import NLIDataset
from esim.utils import correct_predictions
from esim.model_sim import ESIM
...
def main(train_file,
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        print(
            "-> Average batch processing time: {:.4f}s, total test time: {:.4f}s, p correlation: {:.4f}, s correlation: {:.4f}".format(
                batch_time, total_time, (p * 100), (s * 100)))
        print(40 * "==")
        ...
        if epoch > 10:
            if val_p <= best_score:
                ...
                torch.save({"epoch": epoch,
                           ...
                           os.path.join(target_dir, output + "_" + str(proportion) + "_best_sim.pth.tar"))


            if patience_counter >= patience:
                print("-> Early stopping: patience limit reached, stopping...")
                checkpoint = torch.load(os.path.join(target_dir, output + "_" + str(proportion) + "_best_sim.pth.tar"))
                ...
                os.remove(os.path.join(target_dir, output + "_" + str(proportion) + "_best_sim.pth.tar"))
                break
        if epoch == 30 :
            checkpoint = torch.load(os.path.join(target_dir, output + "_" + str(proportion) + "_best_sim.pth.tar"))
            # Retrieving model parameters from checkpoint.
...
if __name__ == "__main__":
    default_config = "../../config/training/sts_training_sim.json"
