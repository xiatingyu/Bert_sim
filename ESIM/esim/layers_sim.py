...
import torch
from .utils import sort_by_seq_lens, masked_softmax_sim, weighted_sum, masked_softmax
...
class SoftmaxAttention(nn.Module):
    ...
    def forward(self,
        ...
        left = similarity_matrix.shape[1]
        right = similarity_matrix.shape[2]

        similarity_matrix = torch.mul(similarity_matrix, similarity[ : , :left, :right])
        ...
        # similarity = similarity[:, :left, :right]
        prem_hyp_attn = masked_softmax_sim(similarity_matrix, hypothesis_mask, similarity)
        hyp_prem_attn = masked_softmax_sim(similarity_matrix.transpose(1, 2).contiguous(),premise_mask, similarity.transpose(1, 2).contiguous())'''
