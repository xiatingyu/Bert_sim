...
def sort_by_seq_lens(batch, sequences_lengths, descending=True):
    ...
    return sorted_batch, sorted_seq_lens.cpu(), sorting_index, restoration_index
...
def masked_softmax_sim(tensor, mask, similarity):
    """
    Apply a masked softmax on the last dimension of a tensor.
    The input tensor and mask should be of size (batch, *, sequence_length).
    ...
    tensor_shape = tensor.size()
    reshaped_tensor = tensor.view(-1, tensor_shape[-1])
    similarity_shape = similarity.size()
    reshaped_similarity = similarity.contiguous().view(-1, similarity_shape[-1])
    ...
    result = nn.functional.softmax(reshaped_tensor * reshaped_mask, dim=-1)
    result = torch.mul(result, reshaped_similarity)
    #result = torch.mul(result, reshaped_similarity)
    result = result * reshaped_mask
    # 1e-13 is added to avoid divisions by zero.
    result = result / (result.sum(dim=-1, keepdim=True) + 1e-13)
